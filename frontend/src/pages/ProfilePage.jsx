...
import { ChevronLeft, Building2, Mail, ShieldCheck, Save, CheckCircle } from 'lucide-react';
          ...
          <div className="flex items-center justify-end gap-4">
            <div>
              ...
              <div className="flex items-center justify-end gap-2 mt-2">
                {isOwner && <span className="text-xs font-semibold px-2 py-0.5 rounded-md bg-white/10 text-white border border-white/20">مالك مستودع</span>}
                {isRenter && <span className="text-xs font-semibold px-2 py-0.5 rounded-md bg-white/10 text-white border border-white/20">مستأجر</span>}
              </div>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 text-right">
          ...
          <form onSubmit={handleUpdateCompany} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={userInfo.commercial_registration || ''}
                disabled
                className="w-full py-2.5 px-3.5 rounded-xl text-sm font-medium text-right bg-gray-50 border border-gray-100 text-gray-400 cursor-not-allowed"
                placeholder="غير متاح للتعديل" />
              <p className="text-xs text-gray-400 mt-1">رقم السجل التجاري لا يمكن تعديله</p>
            </div>
